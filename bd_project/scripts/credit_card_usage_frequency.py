...
filtered_valid_records = field.filter(lambda x: x[0][1] == "Valid" and x[1][0] == "Valid" and x[1][1] == "1").map(lambda x: (x[0][0],1)).reduceByKey(lambda x,y: x+y)
	
tabSeparated =  filtered_valid_records.map(lambda x: x[0]+"\t"+str(x[1])) 
tabSeparated.saveAsTextFile("credit_card_frequency.out")
