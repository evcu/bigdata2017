...
field = taxi_data.map(lambda entry: (entry[1], entry[fields['fare_amount']])).map(lambda entry: (entry[0],entry[1],checkPickUpDateValid(entry[0]))).filter(lambda x: x[2] == "Valid")
...
tabSeparated.saveAsTextFile("fare_amount_per_date_frequency.out")
