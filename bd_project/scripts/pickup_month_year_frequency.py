...
filtered_valid_records = field.filter(lambda x: x[1] == "Valid").map(lambda x: (x[0],1)).reduceByKey(lambda x,y: x+y).sortByKey()
	
tabSeparated =  filtered_valid_records.map(lambda x: x[0]+"\t"+str(x[1])) 
tabSeparated.saveAsTextFile("pickup_month_year_frequency.out")
